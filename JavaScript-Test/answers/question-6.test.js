 ...
 */
describe("partition", () => {
  it("Should return an empty array if either parameter is given wrong", () => {
    expect(partition()).toEqual([]);
    expect(partition(null, null)).toEqual([]);
    expect(partition([1], null)).toEqual([]);
    expect(partition(null, () => true)).toEqual([]);
  });

  it("Should put the elements in the correct list", () => {
    const testArray = [1, 2];
    const testFunctionOne = (element) => element === 1;
    const testFunctionTwo = (element) => element === 2;

    expect(partition(testArray, testFunctionOne)).toEqual([[1], [2]]);
    expect(partition(testArray, testFunctionTwo)).toEqual([[2], [1]]);
  });
});
